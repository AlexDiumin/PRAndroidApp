...
//            bufferedImages.remove(bufferedImages.size() - 1);
        ...
        for (int i = 0; i < bufferedImagesSize; i++) {
            ...
            for (int h = 0; h < bufferedImages.get(i).getHeight(); h++) {
                for (int w = 0; w < bufferedImages.get(i).getWidth(); w++) {
                    testingPixelsImages[i][0][h*bufferedImages.get(i).getHeight() + w] = BigDecimal.valueOf(new Color(bufferedImages.get(i).getRGB(w, h)).getRed()/255.);
                    testingPixelsImages[i][1][h*bufferedImages.get(i).getHeight() + w] = BigDecimal.valueOf(new Color(bufferedImages.get(i).getRGB(w, h)).getGreen()/255.);
                    testingPixelsImages[i][2][h*bufferedImages.get(i).getHeight() + w] = BigDecimal.valueOf(new Color(bufferedImages.get(i).getRGB(w, h)).getBlue()/255.);
                }
            }
            // смещение
            testingPixelsImages[i][0][bufferedImages.get(i).getHeight()*bufferedImages.get(i).getWidth()] = BigDecimal.ONE;
